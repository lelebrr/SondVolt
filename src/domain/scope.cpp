...
bool scope_capture(ScopeCapture* out, uint32_t timeoutMs) {
    ...
    // Comparar contra millis()+timeout quebra a cada 49 dias, quando o
    // contador da a volta. Subtrair funciona sempre, porque a diferenca de
    // dois unsigned tambem da a volta corretamente.
    const uint32_t start = millis();
    uint8_t attempts = 0;

    while ((millis() - start) < timeoutMs && attempts < 8) {
        attempts++;
...
RippleResult ripple_measure() {
    ...
    //
    // A versao anterior amostrava DUAS vezes o mesmo sinal: uma passada para
    // achar a media e outra para o RMS e a frequencia. Como o sinal muda
    // entre elas, a media usada no RMS nao era a media daquelas amostras.
    // Alem disso o tempo entre amostras era assumido como 78 us fixos, um
    // chute - delayMicroseconds nao garante isso e o overhead do ADC entra
    // por cima. Agora e UMA passada, guardando as amostras, e o tempo real
    // e medido com micros().
    expander_ripple_coupling(true);

    static uint16_t samples[RIPPLE_SAMPLES];
    uint16_t rawMin = ADC_MAX_COUNT, rawMax = 0;
    double sum = 0.0;

    const uint32_t tStart = micros();
    for (uint16_t i = 0; i < RIPPLE_SAMPLES; i++) {
    ...
    const uint32_t tElapsed = micros() - tStart;

    expander_ripple_coupling(false);

    // Intervalo REAL entre amostras, nao o nominal.
    const float dt = (RIPPLE_SAMPLES > 1)
                   ? ((float)tElapsed * 1e-6f / (float)(RIPPLE_SAMPLES - 1))
                   : 1e-4f;

    const float mean = (float)(sum / RIPPLE_SAMPLES);

    // RMS e cruzamentos sobre as MESMAS amostras que geraram a media.
    double sumSq = 0.0;
    uint16_t crossings = 0;
    uint32_t firstCross = 0, lastCross = 0;
    bool above = ((float)samples[0] > mean);
    ...
    for (uint16_t i = 0; i < RIPPLE_SAMPLES; i++) {
        ...
        if (above && v < mean - 15.0f) {
            above = false;
            if (crossings == 0) firstCross = i;
            lastCross = i; crossings++;
        } else if (!above && v > mean + 15.0f) {
            above = true;
            if (crossings == 0) firstCross = i;
            lastCross = i; crossings++;
        }
    ...
    float rmsRaw = (float)sqrt(sumSq / RIPPLE_SAMPLES);
    r.rippleRms = (rmsRaw / (float)ADC_MAX_COUNT) * hal_adc_vref();

    if (crossings >= 2 && lastCross > firstCross) {
        float cycles  = (float)(crossings - 1) / 2.0f;
        float seconds = (float)(lastCross - firstCross) * dt;
...
ZenerResult zener_measure() {
    ...
    // Se a leitura passar da tensao da fonte (ruido, ou divisor mal
    // calibrado), a conta daria corrente negativa - o que nao existe neste
    // circuito. Satura em zero e o veredito abaixo trata o caso.
    float drop = BOOST_OUTPUT_VOLTS - measured;
    if (drop < 0.0f) drop = 0.0f;
    z.testCurrentMa = (drop / ZENER_SERIES_RESISTOR) * 1000.0f;
