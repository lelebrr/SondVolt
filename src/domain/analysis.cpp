...
float analysis_measure_leakage() {
    ...
    // A capacitancia REAL da peca precisa ser medida antes: a versao anterior
    // usava o offset parasita dos cabos (algumas dezenas de pF) como se fosse
    // o capacitor sob teste, e o resultado saia varias ordens de grandeza
    // errado. Sem capacitor conectado nao ha fuga que faca sentido medir.
    float capacitance = analysis_measure_capacitance();
    if (capacitance < 1e-10f) return ANALYSIS_OPEN;
...
bool analysis_resistor_color_bands(float ohms, const char* band[4]) {
    ...
    // NaN faz QUALQUER comparacao dar falso, entao "ohms < 1 || ohms > 99e6"
    // deixava NaN passar. Depois disso, (int)NaN e comportamento indefinido e
    // o indice do vetor de cores saia da faixa. O teste positivo abaixo so
    // aceita um numero de verdade dentro do intervalo.
    if (isnan(ohms) || isinf(ohms))            return false;
    ...
    if (digits < 10) digits = 10;    // a normalizacao garante 10..99
