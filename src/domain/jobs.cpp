...
void jobs_init() {
    ...
    if (gPrefs.begin("jobs", true)) {
        char last[JOB_MAX_DIR + 2] = { 0 };
        // O nome ocupa ate JOB_MAX_DIR caracteres MAIS o terminador. Ler
        // sizeof-1 cortava o ultimo caractere de um nome de 8 letras.
        size_t n = gPrefs.getBytes("active", last, JOB_MAX_DIR + 1);
        gPrefs.end();
