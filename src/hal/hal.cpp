...
bool hal_bus_is_shared(HalBus bus) {
#if SONDVOLT_HW_REV == 0
    return (bus == HAL_BUS_ONEWIRE || bus == HAL_BUS_DISCHARGE ||
            bus == HAL_BUS_PROBE_DRIVE);
#else
    return (bus == HAL_BUS_PROBE_DRIVE);   // I2C sempre divide com a excitacao
#endif
}

bool hal_bus_acquire(HalBus bus, uint32_t timeoutMs) {
    // O enum e signed: sem o teste do limite inferior, um -1 vindo de um
    // cast errado indexaria gBus[-1].
    if ((int)bus < 0 || bus >= HAL_BUS_COUNT) return false;
    BusState& b = gBus[bus];
...
void hal_bus_release(HalBus bus) {
    if ((int)bus < 0 || bus >= HAL_BUS_COUNT) return;
    BusState& b = gBus[bus];
    if (!b.mutex) return;
...
bool hal_bus_is_busy(HalBus bus) {
    return ((int)bus >= 0 && bus < HAL_BUS_COUNT) ? gBus[bus].busy : false;
}
...
float hal_adc_to_volts(uint16_t raw) {
    ...
    // Os coeficientes chegam a 1e-14: em float (7 digitos significativos) o
    // termo de quarta ordem perde toda a precisao. O acumulo e feito em
    // double e so o resultado volta para float.
    const double x = (double)raw;
    double poly = -0.000000000000016 * x * x * x * x
                +  0.000000000118171 * x * x * x
                -  0.000000301211691 * x * x
                +  0.001109019271794 * x
                +  0.034143524634089;

    float corrected = (float)poly;
...
#endif
    // PROBE_DRIVE e I2C sao o mesmo enum agora: uma configuracao so.
