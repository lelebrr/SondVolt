...
enum HalBus {
    ...
    // ATENCAO: PROBE_DRIVE e I2C sao o MESMO recurso fisico.
    // GPIO27 = SDA = drive de 10k, GPIO22 = SCL = drive de 470R.
    // Ate a v5.0 eles tinham mutexes separados, o que nao impedia nada: o
    // INA219 podia iniciar uma transacao no exato instante em que a
    // excitacao das pontas puxava o mesmo pino. Agora compartilham o mesmo
    // mutex, e a exclusao e real.
    HAL_BUS_PROBE_DRIVE,
    HAL_BUS_I2C = HAL_BUS_PROBE_DRIVE,   // alias intencional

    HAL_BUS_COUNT = HAL_BUS_PROBE_DRIVE + 1
};
...
// Verdadeiro se dois recursos disputam o mesmo pino nesta revisao. Usado
// pelas telas de diagnostico para explicar a arbitragem ao usuario.
