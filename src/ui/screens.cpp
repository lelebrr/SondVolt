...
// Botao das telas de instrumento. Delega para ui_button(), que e o mesmo
// botao usado em todo o resto da interface - inclusive a cor do texto, que
// ui_button() calcula pela luminancia do fundo em vez de receber pronta.
static void button(int16_t x, int16_t y, int16_t w, int16_t h,
                   const char* label, uint16_t bg, uint16_t fg) {
    (void)fg;   // ui_button escolhe sozinho o que da contraste
    const bool filled = (bg != V_BG_SURFACE);
    ui_button(x, y, w, h, label, filled ? bg : th_accent, filled);
}
...
static void draw_missing_hardware(const char* title, const char* whatIsMissing,
    ...
    ui_surface(TH_MARGIN, 56, SCREEN_WIDTH - TH_MARGIN * 2, 136,
               th_bg1, th_mix(TH_WARNING, th_bg1, 140), TH_RADIUS_LG);

    LOCK_TFT();
    // Triangulo de atencao desenhado com contorno duplo: fica nitido mesmo
    // sem antisserrilhado.
    tft.fillTriangle(160, 72, 147, 95, 173, 95, TH_WARNING);
    tft.drawTriangle(160, 72, 147, 95, 173, 95, th_light(TH_WARNING, 60));
    draw_text_5x7(tft, 158, 84, "!", th_bg0, TH_FONT_BODY);

    centered(160, 106, "RECURSO INDISPONIVEL", TH_WARNING, TH_FONT_CAPTION);
    UNLOCK_TFT();

    ui_section(TH_MARGIN + TH_SP_3, 122, SCREEN_WIDTH - TH_MARGIN * 2 - TH_SP_6,
               nullptr);

    LOCK_TFT();
    centered(160, 132, whatIsMissing, th_txHi, TH_FONT_BODY);
    centered(160, 150, howToFix, th_txLo, TH_FONT_CAPTION);
    centered(160, 172, "Detalhes em docs/WIRING.md", th_txDim, TH_FONT_CAPTION);
    UNLOCK_TFT();
}
...
void screen_scope_draw() {
    ...
    if (gCapture.valid && gCapture.count > 1) {
        ...
        // O laco abaixo divide por (count - start). A condicao de parada ja
        // garante start < count, mas deixar o invariante explicito custa uma
        // linha e protege de uma refatoracao futura.
        if (start >= gCapture.count) start = 0;
        const uint16_t span = (uint16_t)(gCapture.count - start);

        for (uint16_t i = 0; i < span; i++) {
            uint16_t idx = start + i;
            int16_t px = gx + 1 + (int16_t)(((int32_t)i * (gw - 2)) / span);
            if (px >= gx + gw - 1) break;
...
void screen_ripple_draw() {
    ...
    ui_card_titled(TH_MARGIN, 46, 145, 78, "ONDULACAO", accent);
    ui_big_value(TH_MARGIN, 56, 145, 62, val, "% do DC", accent);

    analysis_format_eng(gRipple.dcVolts, buf, sizeof(buf), 2);
    strncat(buf, "V", sizeof(buf) - strlen(buf) - 1);
    ui_card_titled(SCREEN_WIDTH - 145 - TH_MARGIN, 46, 145, 78,
                   "TRILHO DC", th_accent);
    ui_big_value(SCREEN_WIDTH - 145 - TH_MARGIN, 56, 145, 62, buf,
                 "medio", th_accent);
