...
// ============================================================================
// Estes substituem o desenho manual de retangulo espalhado pelas telas.
// Usando-os, uma mudanca de estilo acontece num lugar so.

// Superficie base de qualquer painel: sombra sutil, canto arredondado e
// borda de um passo acima do fundo.
void ui_surface(int16_t x, int16_t y, int16_t w, int16_t h,
                uint16_t fill, uint16_t border, uint8_t radius);

// Cartao com faixa de acento no topo. E a peca visual mais repetida da
// interface: painel de valor, bloco de informacao, item destacado.
void ui_card(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t accent);

// Cartao com titulo pequeno em maiuscula sobre a faixa de acento.
void ui_card_titled(int16_t x, int16_t y, int16_t w, int16_t h,
                    const char* title, uint16_t accent);

// Botao com estado. `pressed` desenha a versao afundada.
void ui_button(int16_t x, int16_t y, int16_t w, int16_t h,
               const char* label, uint16_t color, bool filled = true,
               bool pressed = false);

// Etiqueta arredondada com ponto colorido. Para status curto e legivel.
void ui_chip(int16_t x, int16_t y, const char* text, uint16_t color);

// Rotulo de secao com linha que preenche o resto da largura.
void ui_section(int16_t x, int16_t y, int16_t w, const char* title);

// Par rotulo/valor alinhado, o bloco de informacao mais comum das telas.
void ui_field(int16_t x, int16_t y, int16_t w,
              const char* label, const char* value, uint16_t valueColor);

// Cabecalho padrao: seta de voltar, titulo e um indicador opcional a direita.
void ui_header(const char* title, const char* rightBadge = nullptr,
               uint16_t badgeColor = 0);

// Barra de estado do rodape: cartao, relogio, trabalho ativo, rede.
void ui_statusbar();

// Valor grande com unidade, escolhendo a maior fonte que couber.
void ui_big_value(int16_t x, int16_t y, int16_t w, int16_t h,
                  const char* value, const char* unit, uint16_t color);

// ============================================================================
// 12. CABECALHOS E CARTOES (nomes antigos)
// ============================================================================
