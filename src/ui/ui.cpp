    ...
    switch (currentAppState) {
        ...
        case STATE_SCANNER: {
            ...
            if (res.valid && res.type != COMP_NONE) {
                snprintf(valBuf, sizeof(valBuf), "%s", res.valueText);
...
static void draw_status_bar() {
    // Cartao, trabalho ativo, rede e relogio - tudo em ui_statusbar(), que
    // e a mesma barra em todas as telas.
    ui_statusbar();
}
    ...
    if (isTouching) {
        ...
        if (isMenuMode) {
            ...
            if (!hasMoved) {
                ...
                // Divisao inteira trunca para zero: -10/45 da 0, e um toque
                // no cabecalho acionaria o primeiro item da lista.
                if (relativeY < 0) return;
                int16_t itemIdx = relativeY / SETTINGS_ITEM_HEIGHT;
...
static void draw_cpu_info_screen() {
    ...
    // ------------------------------------------------------- barra de memoria
    // O widget faz o proprio lock; aqui ja estamos fora da secao critica.
...
static void apply_theme(uint8_t idx) {
    // O indice vem de um contador de toques. O modulo garante que ele nunca
    // saia da lista de acentos, por mais que o usuario insista no botao.
    deviceSettings.themeIdx = (uint8_t)(idx % TH_ACCENT_COUNT);
    colors_update();
    settings_mark_dirty();
}
