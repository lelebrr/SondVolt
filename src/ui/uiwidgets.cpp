...
// ============================================================================
// 11. CARTOES E SUPERFICIES (sistema de design da v5.0)
// ============================================================================
// Antes disto, cada tela desenhava seu proprio retangulo com sua propria
// margem e seu proprio raio. O resultado era uma interface que parecia
// montada por pessoas diferentes. Aqui a forma e uma so.

void ui_surface(int16_t x, int16_t y, int16_t w, int16_t h,
                uint16_t fill, uint16_t border, uint8_t radius) {
    LOCK_TFT();

    // Sombra de um pixel deslocada. Numa TFT sem transparencia, uma linha
    // escura embaixo e a direita ja da a sensacao de camada.
    tft.drawRoundRect(x + 1, y + 1, w, h, radius, th_shade(th_bg0, 90));
...
void ui_card(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t accent) {
    ui_surface(x, y, w, h, th_bg1, th_bg3, TH_RADIUS_MD);

    LOCK_TFT();
    // Faixa de acento de 3 px no topo, com os cantos respeitando o raio.
    tft.fillRect(x + TH_RADIUS_MD, y, w - TH_RADIUS_MD * 2, 3, accent);
    tft.drawFastHLine(x + TH_RADIUS_MD - 2, y + 1, 2,
                      th_mix(accent, th_bg1, 120));
    tft.drawFastHLine(x + w - TH_RADIUS_MD, y + 1, 2,
                      th_mix(accent, th_bg1, 120));
    UNLOCK_TFT();
}

void ui_card_titled(int16_t x, int16_t y, int16_t w, int16_t h,
                    const char* title, uint16_t accent) {
    ui_card(x, y, w, h, accent);

    if (title && title[0]) {
        LOCK_TFT();
        draw_text_5x7(tft, x + TH_SP_2, y + TH_SP_2, title, accent,
                      TH_FONT_CAPTION);
        UNLOCK_TFT();
    }
}

void ui_button(int16_t x, int16_t y, int16_t w, int16_t h,
               const char* label, uint16_t color, bool filled, bool pressed) {
    // Pressionado escurece e "afunda" um pixel: o feedback tatil que uma tela
    // resistiva nao consegue dar de outro jeito.
    const int16_t oy = pressed ? 1 : 0;
    const uint16_t bg = filled
                      ? (pressed ? th_shade(color, 60) : color)
                      : (pressed ? th_bg2 : th_bg1);
    const uint16_t fg = filled ? th_on(bg) : color;

    LOCK_TFT();
    if (!pressed) {
        tft.drawRoundRect(x + 1, y + 2, w, h, TH_RADIUS_SM,
                          th_shade(th_bg0, 90));
    }
    tft.fillRoundRect(x, y + oy, w, h, TH_RADIUS_SM, bg);
    tft.drawRoundRect(x, y + oy, w, h, TH_RADIUS_SM,
                      filled ? th_light(bg, 40) : th_mix(color, th_bg1, 120));

    if (label && label[0]) {
        const int16_t tw = TH_TEXT_W(label, TH_FONT_BODY);
        draw_text_5x7(tft, x + (w - tw) / 2,
                      y + oy + (h - TH_CHAR_H(TH_FONT_BODY)) / 2 + 1,
                      label, fg, TH_FONT_BODY);
    }
    UNLOCK_TFT();
}

void ui_chip(int16_t x, int16_t y, const char* text, uint16_t color) {
    if (!text) return;
    const int16_t w = TH_TEXT_W(text, TH_FONT_CAPTION) + TH_SP_5;

    LOCK_TFT();
    tft.fillRoundRect(x, y, w, TH_CHIP_H, TH_CHIP_H / 2,
                      th_tint(color, th_bg1));
    tft.drawRoundRect(x, y, w, TH_CHIP_H, TH_CHIP_H / 2,
                      th_mix(color, th_bg1, 110));
    tft.fillCircle(x + TH_SP_2 - 1, y + TH_CHIP_H / 2, 2, color);
    draw_text_5x7(tft, x + TH_SP_3 + 1, y + (TH_CHIP_H - 7) / 2,
                  text, th_txMd, TH_FONT_CAPTION);
    UNLOCK_TFT();
}

void ui_section(int16_t x, int16_t y, int16_t w, const char* title) {
    LOCK_TFT();
    if (title && title[0]) {
        draw_text_5x7(tft, x, y, title, th_accent, TH_FONT_CAPTION);
        const int16_t tw = TH_TEXT_W(title, TH_FONT_CAPTION) + TH_SP_2;
        if (tw < w) {
            // A linha desvanece para a direita, em vez de terminar seca.
            for (int16_t i = x + tw; i < x + w; i++) {
                const uint8_t fade = (uint8_t)(
                    200 - ((i - (x + tw)) * 160) / (w - tw + 1));
                tft.drawPixel(i, y + 3, th_mix(th_bg3, th_bg0, fade));
            }
        }
    } else {
        tft.drawFastHLine(x, y, w, th_bg3);
    }
    UNLOCK_TFT();
}

void ui_field(int16_t x, int16_t y, int16_t w,
              const char* label, const char* value, uint16_t valueColor) {
    LOCK_TFT();
    if (label) {
        draw_text_5x7(tft, x, y, label, th_txLo, TH_FONT_CAPTION);
    }
    if (value) {
        // Valor alinhado a direita: numeros de comprimentos diferentes ficam
        // com as unidades na mesma coluna, e a leitura fica muito mais rapida.
        const int16_t vw = TH_TEXT_W(value, TH_FONT_BODY);
        draw_text_5x7(tft, x + w - vw, y, value, valueColor, TH_FONT_BODY);
    }
    UNLOCK_TFT();
}

void ui_header(const char* title, const char* rightBadge, uint16_t badgeColor) {
    if (badgeColor == 0) badgeColor = th_accent;

    LOCK_TFT();
    // Fundo do cabecalho com um degrade curto: a barra nao fica chapada e a
    // separacao com o conteudo aparece sem precisar de linha grossa.
    for (int16_t i = 0; i < TH_HEADER_H; i++) {
        const uint8_t k = (uint8_t)((i * 60) / TH_HEADER_H);
        tft.drawFastHLine(0, i, SCREEN_WIDTH, th_mix(th_bg1, th_bg0, k));
    }
    tft.drawFastHLine(0, TH_HEADER_H, SCREEN_WIDTH, th_bg3);
    tft.drawFastHLine(0, TH_HEADER_H + 1, SCREEN_WIDTH,
                      th_mix(th_accent, th_bg0, 190));

    // Seta de voltar: um chevron desenhado, nao um caractere de fonte.
    const int16_t bx = TH_SP_3, by = TH_HEADER_H / 2;
    for (uint8_t t = 0; t < 2; t++) {
        tft.drawLine(bx + 5, by - 5 + t, bx, by + t, th_accent);
        tft.drawLine(bx, by + t, bx + 5, by + 5 + t, th_accent);
    }

    if (title && title[0]) {
        draw_text_5x7(tft, bx + 16, by - 4, title, th_txHi, TH_FONT_BODY);
    }

    if (rightBadge && rightBadge[0]) {
        const int16_t bw = TH_TEXT_W(rightBadge, TH_FONT_CAPTION) + TH_SP_3;
        const int16_t bxr = SCREEN_WIDTH - bw - TH_SP_2;
        tft.fillRoundRect(bxr, by - 7, bw, 14, 7, th_tint(badgeColor, th_bg1));
        draw_text_5x7(tft, bxr + TH_SP_2 - 2, by - 3, rightBadge,
                      badgeColor, TH_FONT_CAPTION);
    }
    UNLOCK_TFT();
}

void ui_statusbar() {
    const int16_t y = SCREEN_HEIGHT - TH_STATUSBAR_H;

    LOCK_TFT();
    tft.fillRect(0, y, SCREEN_WIDTH, TH_STATUSBAR_H, th_bg1);
    tft.drawFastHLine(0, y, SCREEN_WIDTH, th_bg3);

    int16_t x = TH_SP_2;

    // --- Cartao SD: ponto colorido, sem texto quando esta tudo bem ---------
    tft.fillCircle(x + 2, y + TH_STATUSBAR_H / 2, 2,
                   sdCardPresent ? TH_SUCCESS : th_txDim);
    x += TH_SP_3;

    // --- Trabalho ativo: o contexto mais importante da tela ----------------
    if (jobs_has_active()) {
        char job[18];
        snprintf(job, sizeof(job), "%.14s", jobs_active_name());
        draw_text_5x7(tft, x, y + 4, job, TH_SUCCESS, TH_FONT_CAPTION);
        x += TH_TEXT_W(job, TH_FONT_CAPTION) + TH_SP_3;
    }

    // --- Rede: so aparece quando esta ligada -------------------------------
    if (net_is_connected()) {
        draw_text_5x7(tft, x, y + 4, "WiFi", th_accent, TH_FONT_CAPTION);
        x += TH_SP_6 + TH_SP_1;
    }

    // --- Relogio a direita, quando confiavel -------------------------------
    if (net_time_valid()) {
        char clock[10];
        net_format_time(clock, sizeof(clock));
        clock[5] = '\0';                       // so hh:mm cabe com folga
        const int16_t cw = TH_TEXT_W(clock, TH_FONT_CAPTION);
        draw_text_5x7(tft, SCREEN_WIDTH - cw - TH_SP_2, y + 4,
                      clock, th_txLo, TH_FONT_CAPTION);
    } else {
        // Sem relogio, mostra a versao: um espaco vazio parece defeito.
        const char* v = "v" FW_VERSION;
        const int16_t vw = TH_TEXT_W(v, TH_FONT_CAPTION);
        draw_text_5x7(tft, SCREEN_WIDTH - vw - TH_SP_2, y + 4,
                      v, th_txDim, TH_FONT_CAPTION);
    }
    UNLOCK_TFT();
}

void ui_big_value(int16_t x, int16_t y, int16_t w, int16_t h,
                  const char* value, const char* unit, uint16_t color) {
    const char* v = (value && value[0]) ? value : "---";

    // Maior escala que ainda cabe, descontando a margem lateral.
    uint8_t size = TH_FONT_HERO;
    while (size > TH_FONT_BODY &&
           TH_TEXT_W(v, size) > (w - TH_SP_4)) {
        size--;
    }

    const int16_t tw = TH_TEXT_W(v, size);
    const int16_t th = TH_CHAR_H(size);
    const int16_t tx = x + (w - tw) / 2;
    const int16_t ty = y + (h - th) / 2;

    LOCK_TFT();
    // Sombra de um pixel na diagonal. Sob luminaria de bancada, isso e a
    // diferenca entre ler o valor de relance e precisar focar.
    draw_text_5x7(tft, tx + 1, ty + 1, v, th_shade(color, 170), size);
    draw_text_5x7(tft, tx, ty, v, color, size);

    if (unit && unit[0]) {
        // A unidade acompanha a linha de base do numero, alinhada a direita.
        const int16_t uw = TH_TEXT_W(unit, TH_FONT_CAPTION);
        draw_text_5x7(tft, x + w - uw - TH_SP_2, ty + th - 7,
                      unit, th_txLo, TH_FONT_CAPTION);
    }
    UNLOCK_TFT();
}
