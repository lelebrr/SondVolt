...
static void handle_status_json() {
    // multimeter_get_last_reading() devolve uma COPIA por valor. A estrutura
    // e atualizada pela tarefa de medicao no outro nucleo, entao a copia pode
    // pegar campos de dois instantes diferentes. Para uma pagina informativa
    // isso e aceitavel; o que nao se pode e guardar ponteiro para ela.
...
static void stream_sd_file(const char* path, const char* downloadName) {
    ...
    // O mutex e tomado apenas em volta de cada LEITURA do cartao, nunca em
    // volta do envio pela rede. Segurar o display durante um download inteiro
    // (que pode levar segundos numa rede ruim) congelava a interface.
    LOCK_TFT();
    FsFile f = sd.open(path, FILE_READ);
    bool opened = (bool)f;
    uint32_t total = opened ? f.size() : 0;
    UNLOCK_TFT();
    ...
    // Blocos de 512 bytes: casa com o setor do cartao e cabe folgado na RAM.
    uint8_t  buffer[512];
    uint32_t sent = 0;
    ...
    while (sent < total) {
        ...
        size_t got = 0;
        int c;
        while (got < sizeof(buffer) && (c = f.read()) >= 0) {
        ...
        // write() pode enviar menos que o pedido quando o buffer TCP enche.
        // Ignorar o retorno truncava o arquivo em silencio.
        size_t written = 0;
        uint8_t retries = 0;
        while (written < got && retries < 40) {
            size_t n = gServer.client().write(buffer + written, got - written);
            if (n == 0) {
                retries++;
        ...
        if (written < got) break;   // cliente desistiu
        sent += written;
    }
