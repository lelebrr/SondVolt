...
// Inverte um trecho do vetor no lugar.
static void reverse_range(HistoryItem* a, uint8_t from, uint8_t to) {
    while (from < to) {
        HistoryItem tmp = a[from];
        a[from] = a[to];
        a[to]   = tmp;
        from++;
        to--;
...
// Rotaciona o vetor k posicoes para a esquerda, sem memoria auxiliar.
// Reverte o inicio, reverte o fim, reverte tudo.
static void rotate_left(HistoryItem* a, uint8_t n, uint8_t k) {
    if (n < 2) return;
    k %= n;
    if (k == 0) return;
    reverse_range(a, 0, (uint8_t)(k - 1));
    reverse_range(a, k, (uint8_t)(n - 1));
    reverse_range(a, 0, (uint8_t)(n - 1));
}
...
uint8_t logger_get_recent(HistoryItem* buffer, uint8_t maxEntries) {
    ...
    //
    // A versao anterior usava um temporario fixo de 16 posicoes e fazia o
    // modulo sobre ele: com maxEntries acima de 16 a rotacao embaralhava o
    // historico em vez de ordena-lo. Agora a rotacao e feita no proprio
    // buffer, por reversao tripla - nao precisa de memoria auxiliar nenhuma
    // e funciona para qualquer tamanho.
    uint8_t start = (uint8_t)(count % maxEntries);
    if (start != 0) {
        rotate_left(buffer, maxEntries, start);
    }
    return maxEntries;
}
