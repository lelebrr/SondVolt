...
bool tcam_read_frame(ThermalFrame* out) {
    ...
    // A versao anterior inicializava minC e maxC com pixels[0]. Se aquele
    // pixel especifico viesse NaN - o que acontece com pixel defeituoso do
    // sensor - TODAS as comparacoes seguintes davam falso e a imagem inteira
    // ficava sem escala. Agora a semente e o primeiro pixel VALIDO.
    double  sum      = 0.0;
    uint16_t nValid  = 0;
    bool    seeded   = false;
    ...
    for (uint16_t i = 0; i < TCAM_PIXELS; i++) {
        ...
        if (isnan(t) || isinf(t) || t < -40.0f || t > 300.0f) {
            continue;   // trata na segunda passada
        }

        if (!seeded) {
            gFrame.minC = gFrame.maxC = t;
            gFrame.coldIndex = gFrame.hotIndex = i;
            seeded = true;
        } else {
    ...
    // Quadro inteiro invalido: melhor recusar que mostrar uma imagem falsa.
    if (!seeded || nValid == 0) {
        gFrame.valid = false;
        LOG_SERIAL_F("[CAM] Quadro descartado: nenhum pixel valido");
        return false;
    }

    // Substitui os pixels invalidos pelo minimo, agora que ele e conhecido.
    for (uint16_t i = 0; i < TCAM_PIXELS; i++) {
        float t = gFrame.pixels[i];
        if (isnan(t) || isinf(t) || t < -40.0f || t > 300.0f) {
    ...
    // A media divide pelos pixels que realmente contribuiram, nao pelos 768.
    gFrame.avgC    = (float)(sum / (double)nValid);
